<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@include file="rsmUpdateFormscr.jsp"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
	...
	<div class="container">
		...
		<form action="rsmInsert" method="post" enctype="multipart/form-data">
			<input style="font-size: 25px; margin-bottom: 10px" size="60"
				...
				placeholder="기업에게 나에 대해 알려줍시다." value="${rsm.rSubject }">
			<div class="all">
			<div class="content">
				<h3>인적사항</h3>
				<table>
					<tr>
						<td><input type="text" name="rName" placeholder="이름" value="${rsm.rName }"
							size="15"></td>
						<td><input type="date" name="rBirth" placeholder="생년월일"value="${rsm.rBirth }"
							size="15"></td>

						<td><input type="email" name="rEmail" placeholder="이메일"value="${rsm.rEmail }"
							size="15"></td>
						<td><input type="text" name="rTel" placeholder="휴대폰번호"value="${rsm.rTel }"
							size="15" height="3"></td>
					...
					<tr>
						<td colspan="2"><input onclick="execDaumPostcode()" type="text"
							name="address" id="address" placeholder="주소" size="35" value="${rsm.rAddress }"></td>
						...
						<td><select name="rGender" style="width: 100px;">
								<c:if test="${rsm.rGender eq'f' }">
								<option value="f">여성</option><option value="m">남성</option></c:if>
								<c:if test="${rsm.rGender eq 'm' }">
								<option value="m">남성</option><option value="f">여성</option>
								</c:if>
								<c:if test="${rsm.rGender ne 'm'&&rsm.rGender ne 'f' }"><option value="">성별</option>
								<option value="m">남성</option><option value="f">여성</option>
								</c:if>
						</select>
						<select name="rMilitary" style="width: 100px;">
								<c:if test="${rsm.rMilitary eq 'n' }">
								<option value="n" selected="selected">미필</option><option value="y">군필</option>
								<option value="x">해당없음</option></c:if>
								<c:if test="${rsm.rMilitary eq 'y' }">
								<option value="n">미필</option><option value="y" selected="selected">군필</option>
								<option value="x">해당없음</option></c:if>
								<c:if test="${rsm.rMilitary eq 'x' }">
								<option value="n">미필</option><option value="y">군필</option>
								<option value="x" selected="selected">해당없음</option></c:if>
								<c:if test="${rsm.rMilitary ne 'y' && rsm.rMilitary ne 'n' &&rsm.rMilitary ne 'x'}">
								<option value="">병역</option><option value="n">미필</option><option value="y">군필</option>
								<option value="x">해당없음</option></c:if>
						</select></td>
					</tr>
				</table>
			...
			<div class="content">
				...
				<table>
					<caption>
						고등학교<br> 
						<c:if test="${rsm.rHighname eq 'ged' }"><input style="text-align: right;" type="checkbox"
							name="rHighname" checked="checked" value="ged">검정고시 졸업</c:if>
						<c:if test="${rsm.rHighname ne 'ged' }"><input style="text-align: right;" type="checkbox"
							name="rHighname" value="ged">검정고시 졸업</c:if>
					</caption>
					<tr>
						<td><input type="text" name="rHighname" placeholder="학교명" value="${rsm.rHighname }"
							size="40"></td>
						<td><input type="date" name="rHighadmi" placeholder="입학년월" value="${rsm.rHighadmi }"
							size="10"><input type="date" name="rHighgradu" value="${rsm.rHighgradu }"
							placeholder="졸업년월" size="10"></td>
						<td><select name="rHighmajor" style="width: 150px;">
							<c:if test="${rsm.rHighmajor eq '이과계열' }"><option value="이과계열" selected="selected">이과계열</option>
								<option value="문과계열">문과계열</option><option value="전문(실업)계">전문(실업)계</option>
								<option value="예체능계">예체능계</option><option value="특성화/마이스터">특성화/마이스터</option></c:if>
							<c:if test="${rsm.rHighmajor eq '문과계열' }"><option value="이과계열">이과계열</option>
								<option value="문과계열"selected="selected">문과계열</option><option value="전문(실업)계">전문(실업)계</option>
								<option value="예체능계">예체능계</option><option value="특성화/마이스터">특성화/마이스터</option></c:if>
							<c:if test="${rsm.rHighmajor eq '전문(실업)계' }"><option value="이과계열">이과계열</option>
								<option value="문과계열">문과계열</option><option value="전문(실업)계"selected="selected">전문(실업)계</option>
								<option value="예체능계">예체능계</option><option value="특성화/마이스터">특성화/마이스터</option></c:if>
							<c:if test="${rsm.rHighmajor eq '예체능계' }"><option value="이과계열">이과계열</option>
								<option value="문과계열">문과계열</option><option value="전문(실업)계">전문(실업)계</option>
								<option value="예체능계"selected="selected">예체능계</option><option value="특성화/마이스터">특성화/마이스터</option></c:if>
							<c:if test="${rsm.rHighmajor eq '특성화/마이스터' }"><option value="이과계열">이과계열</option>
								<option value="문과계열">문과계열</option><option value="전문(실업)계">전문(실업)계</option>
								<option value="예체능계">예체능계</option><option value="특성화/마이스터"selected="selected">특성화/마이스터</option></c:if>
							<c:if test="${rsm.rHighmajor ne '이과계열' &&rsm.rHighmajor ne '문과계열' &&rsm.rHighmajor ne '전문(실업)계' && rsm.rHighmajor ne '예체능계'&&rsm.rHighmajor ne '특성화/마이스터'}">
							<option value="">전공 계열</option><option value="이과계열">이과계열</option>
								<option value="문과계열">문과계열</option><option value="전문(실업)계">전문(실업)계</option>
								<option value="예체능계">예체능계</option><option value="특성화/마이스터">특성화/마이스터</option></c:if>
						</select></td>
					</tr>
				...
				<table>
					<caption>대학교</caption>
					<tr><td><select name="rUnidivi">
								<c:if test="${rsm.rUnidivi ne '대학교(4년)'&&rsm.rUnidivi ne '대학교(2/3년)'}">
								<option value="">대학구분</option>	<option value="대학교(4년)">대학교(4년)</option>
								<option value="대학교(2/3년)">대학교(2/3년)</option></c:if>
								<c:if test="${rsm.rUnidivi eq '대학교(4년)'}">
								<option value="대학교(4년)" selected="selected">대학교(4년)</option>
								<option value="대학교(2/3년)">대학교(2/3년)</option></c:if>
								<c:if test="${rsm.rUnidivi eq '대학교(2/3년)'}">
								<option value="대학교(4년)">대학교(4년)</option>
								<option value="대학교(2/3년)" selected="selected">대학교(2/3년)</option></c:if>
						</select></td>
						<td><input type="text" name="rUniname" placeholder="학교명" value="${rsm.rUniname }"
							size="20"></td>
						<td><input type="date" name="rUniadmi" placeholder="입학년월" value="${rsm.rUniadmi }"
							size="10"><input type="date" name="rUnigradu" value="${rsm.rUnigradu }"
							placeholder="졸업년월" size="10"></td>
					</tr>
					<tr><td colspan="2"><input type="text" name="rUnimajor" value="${rsm.rUnimajor }"
							placeholder="전공과목" size="35"></td>
						<td><input type="text" name="rUnicredit" placeholder="학점" value="${rsm.rUnicredit }"size="10">
						 <c:if test="${rsm.rUnitime ne'a'&& rsm.rUnitime ne 'p' }"><input type="radio" name="rUnitime" value="a">주간
							<input type="radio" name="rUnitime" value="p">야간</c:if>
							 <c:if test="${rsm.rUnitime eq 'a' }"><input type="radio" name="rUnitime" checked="checked" value="a">주간
							<input type="radio" name="rUnitime" value="p">야간</c:if>
							 <c:if test="${rsm.rUnitime eq 'p' }"><input type="radio" name="rUnitime" value="a">주간
							<input type="radio" name="rUnitime" checked="checked" value="p">야간</c:if>
							</td>
					</tr>
					<tr><td colspan="3" style="text-align: center;"><input
				...
				<input type="hidden" id="rUniminor" name="rUniminor"
					placeholder="부전공과목" size="25"value=${rsm.rUniminor }> <input type="hidden" 
					id="rUnithesis" name=rUnithesis placeholder="졸업작품&졸업논문" value="${rsm.rUnithesis }" size="50">
			</div>
			...
			<div class="content" id="license">
				<h3>자격증</h3>
				<c:if test="${!empty licen}"><table id="enterli">			
				<c:forEach var="licen" items="${licen }">
				<tr><td><input type="text" name="rLicense" placeholder="자격증 명" value="${licen.rLicense }"></td>
				<td><input type="text" name="rLicenseissuer"	placeholder="발행처"value="${licen.rLicenseissuer }"></td>
				<td><input type="date" name="rAcqdate" placeholder="취득일"	size="5"value="${licen.rLicenseacq }"></td></tr>
				</c:forEach>
				<tr><td style="text-align: center" colspan="3"><input type="button" value="추가" onclick="plusli()">
				<input type="button" value="삭제" onclick="removeli()"></td></tr>
				</table></c:if>
				<c:if test="${empty licen }"><table id="enterli"style="display: none;"></table></c:if>
			</div>
			<div class="content" id="award">
				<h3>수상</h3>
				<c:if test="${!empty award }"><table id="enteraw">
				<c:forEach var="award" items="${award }">
					<tr><td><input type="text" name="rAwardname" placeholder="수상명" size="30" value="${award.rAwards }"></td>
					<td><input type="text" name="rAwardagency"	placeholder="수상기관" size="20" value="${award.rAwardissuer }"></td>
					<td><input type="date" name="rAwarddate" placeholder="수상연도" size="10" value="${award.rAwarddate }"></td></tr>
					<tr><td  colspan="3"><textarea rows="3" name="rAwarded" cols="91" placeholder="수여 내용 및 결과를 자세히 입력해주세요."> ${award.rAwarded }</textarea></td></tr>
					</c:forEach>
					<tr><td style="text-align: center" colspan="3"><input type="button" value="추가" onclick="plusaw()">
					<input type="button" value="삭제" onclick="removeaw()"></td></tr>
				</table></c:if>
				<c:if test="${empty award }"><table id="enteraw"style="display: none;"></table></c:if>
			</div>
			<div class="content" id="portfolio">
				<h3>포토폴리오</h3>
				<c:if test="${!empty port }"><table id="enterport">
				<c:forEach var="port" items="${port }">
				<tr><td><input type="text" style="height: 50px; width: 400px;"name="url" placeholder="URL 추가" value="${port.url }"></td>
				<td><input type="text" disabled="disabled" value="파일 이름" id="filename" value="${port.rPortfolio  }"><label id="portupload" for="portfile">파일 추가</label>
				<input type="file" id="portfile"name="rPortfolio"></td></tr></c:forEach>
				<tr><td style="text-align: center" colspan="2">
				<input type="button" value="추가" onclick="plusport()"><input type="button" value="삭제" onclick="removeport()"></td></tr>
				</table></c:if>
				<c:if test="${empty port }"><table id="enterport"style="display: none;"></table></c:if>
			</div>
			<div class="content" id="self">
				<h3>자기소개서</h3>
				<c:if test="${!empty self }"><table id="enterself"><c:forEach var="self" items="${self }">
					<tr><td><input size="85" type="text"placeholder="제목"name="SelfSubject" value="${self.selfSubject}"></td></tr>
					<tr><td><textarea rows="10" cols="91">${self.selfContent }</textarea></td></tr></c:forEach>
					<tr><td style="text-align: center"><input type="button" value="추가" onclick="plusself()"></td></tr>
				</table></c:if>
				<c:if test="${empty self }"><table id="enterself"style="display: none;"></table></c:if>
			</div>
			...
			<div id="scrollmenu">
				<table class="menu">
					...
					<tr>
						<td colspan="2">
							<c:if test="${rsm.rOpenwhether eq 'n' }"><input type="checkbox" name="rOpenwhether" id="open">공개여부</c:if>			
							<c:if test="${rsm.rOpenwhether eq 'y' }"><input type="checkbox" checked="checked" name="rOpenwhether" id="open">공개여부</c:if>			
						</td>
					</tr>
					<tr>
