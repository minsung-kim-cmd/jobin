...
<div class="company_warp">
	...
	<div class="job_regStatus">
		<span><a href="allJobpost?companyCode=${companyCode}">진행중</a></span><span style="font-weight: bold; font-size:35px;"><a href="allJobpostPro?companyCode=${companyCode}">접수마감</a></span><span><a href="allJobpostEnd?companyCode=${companyCode}">채용종료</a></span>
	</div>
	<div class="content">
			...
			<jsp:useBean id="toDay" class="java.util.Date" />
					<div class="content-posting">
						...
						<div class="posting-edit">
							<span onclick="endConfirm(${jobpostList.jobPostCode})" style="cursor:pointer;">채용종료</span>
						</div>
					</div>
		...
		<c:forEach items="${jobpostList}" varStatus="status">
			...
			<c:if test="${jobpostList[i].regStatus eq 'n '&&today > jobpostList[i].closing}">
					<div class="content-posting">
						<div class="posting-cnt">
							<span>조회수</span>
						...
						<div class="posting-edit">
							<span onclick="endConfirm(${jobpostList[i].jobPostCode})" style="cursor:pointer;">채용종료</span>
						</div>
					</div>
				<c:set var="a" value="true"/>
			</c:if>
...
function endConfirm(code) {
	if (confirm("채용을 종료하시겠습니까?") == true){    //확인
	    location.href="endEmp?jobPostCode="+code;
	}else{   //취소
	    return;
	}
}
